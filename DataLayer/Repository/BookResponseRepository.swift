...
public final class BookResponseRepository: BookResponseRepositoryProtocol {
    
    public init () {}
    
    public func fetchBookResponse<T>(url: URL, completion: @escaping (Result<T, any Error>) -> Void) where T : Decodable {
        ...
        if let apiKey = self.getAPIKey(named: "KAKAO_API_KEY") { // .plist에서 키 읽어오기
            ...
            completion(.failure(NetworkError.apiKeyLoadingFail))
            return
        }
